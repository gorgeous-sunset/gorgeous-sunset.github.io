...
function search() {			// 最短手順の探索を行う関数
	...
	for(z=0; z<30; z++) {						// 初期局面の３０通りの選択をキューに追加
		...
		quewe[z] = { board: board, hashCode: hashCode, thenIndex: z, previousIndex: -1, previousQuewe: -1 };
	}
	...
	while( rear < 3000000 ) {
		...
		for(i=0; i<4; i++) {
			...
			coppy_board = quewe[front].board.concat();				// 盤面の復元
	...
	console.log("300万局面を超えたので探索を打ち切ります");
}
...
function answer( quewe, front ) {
	...
	answer_arr.push( quewe[ front ].thenIndex );
}
