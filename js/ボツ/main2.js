...
function initialize() {				// ハッシュキーの生成と隣接点の設定
	...
	for(z=0; z<30; z++) {
		hashKey[z] = [];
		for(d=0; d<6; d++) hashKey[z][d] = Math.floor(m.random() * 1000000000);
	}
...
function search() {			// 最短手順の探索を行う関数
	var quewe,
		...
		coppy_board,
		z, i, j, len, same,
		init_hashCode = 0, hashCode,
		nowIndex, nextIndex, preIndex,
		nowColor, nextColor, all_rear = 0, 
		front, rear,
	...
	for(z=0; z<30; z++) {						// 初期局面の３０通りの選択をキューに追加
		...
		quewe = [];
		quewe[0] = { board: coppy_board, hashCode: hashCode, thenIndex: z, previousIndex: -1, previousQuewe: -1 };

		front = 0, rear = 1;
		while( rear < 2500000 ) {
			nowIndex = quewe[ front ].thenIndex;
			preIndex = quewe[ front ].previousIndex;
			for(i=0; i<4; i++) {
				nextIndex = adjacent[ nowIndex ][i];
				if( nextIndex == -1 )		break;
				if( nextIndex == preIndex ) continue;
				coppy_board = quewe[ front ].board.concat();
				nowColor	= coppy_board[ nowIndex ];
				nextColor	= coppy_board[ nextIndex ];
				hashCode = quewe[front].hashCode;					// ハッシュコードの復元
				...
				if( Array.isArray( researched[ hashCode ] ) ){				// 同一局面の判定
					...
					for(j=0; j<len; j++) if( researched[ hashCode ][j] == nextIndex ) { same = true; break; }
					if( same ) continue;
				}else{
					researched[ hashCode ] = [];
				}
				...
				quewe[rear] = { board: coppy_board, hashCode: hashCode, thenIndex: nextIndex, previousIndex: nowIndex, previousQuewe: front };

				if( count_combo( coppy_board ) > 6 ) {
					end_time = new Date();
					console.log("解かれた　front:" + front + " rear:" + rear);
					console.log("総変化数：" + (all_rear + rear) );
					console.log("所要時間：" + (end_time - start_time) / 1000 + "秒");
					answer( quewe, rear );
					return;
				}
				rear++;
		...
		all_rear += rear;
		console.log( z + "地点の探索は打ち切り");
	}
	end_time = new Date();
	console.log("解にたどり着けず　探索局面数：" + all_rear );
	console.log("所要時間：" + (end_time - start_time) / 1000 + "秒");
}


function answer( quewe, front ) {
	if( quewe[front].previousQuewe != -1 )
		answer( quewe, quewe[front].previousQuewe );
	console.log( quewe[front].thenIndex );
}
...
GameStartScene = enchant.Class.create(enchant.Scene, {
	initialize: function() {
		...
		solve_button.moveTo( 268, 149 );
		this.addChild( solve_button );
		solve_button.ontouchend = function(){ search(); };

		input_button.width = reset_button.width = shuffle_button.width = solve_button.width = 27;
		input_button.height = reset_button.height = shuffle_button.height = solve_button.height = 42;
...
function removeDrops() {				// ドロップを消す関数
	...
	for(j=0, len2=disappear[i].length;	j<len2;	j++) {
		...
		target.tl.delay(15 * i).fadeOut( 20 ).delay(0).then(function(){
			this.parentNode.removeChild( this );
		});
	}
	window.setTimeout( dropDrops, 720 * len );
}
		...
		if( board[ base ] == 10 ) {
			for(i=0; i<y; i++) {
				...
				if( board[ target ] != 10 ) {
					...
					stampList[ target ].tl.moveTo( STAMP_SIZE * x, STAMP_SIZE * y, 15 ).delay(0).then(function(){
						removeDrops();
					});
					break;
				}
